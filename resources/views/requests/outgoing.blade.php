...
<div class="RequestsTable">
	...
	<div class="RequestsTable__row">
		...
		<div class="Request__actions">
			...
			@else
			<v-form action="{{ url('request/' . $request->request_id . '/hire') }}" method="post">
				<input type="hidden" name="_token" value="{{ csrf_token() }}">
				<button href="{{ url('request/' . $request->request_id) }}" class="Button Button--s Button--success u--linkClean u--mr-8">Huur
				</button>
			</v-form>
			<v-form action="{{ url('request/' . $request->request_id) }}" method="post">
				<input type="hidden" name="_method" value="DELETE">
